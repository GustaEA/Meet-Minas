...
const _elements = {
	...
	sliderImageDescription: document.querySelector(".slider-image__descricao"),
	sliderPrevButton: document.querySelector(".slider-buttons__btn-prev"),
	sliderNextButton: document.querySelector(".slider-buttons__btn-nxt"),
}
...
const updateModal = (imgId) => {
	...
	_elements.sliderThumbsImage.forEach(img => {img.classList.remove("slider-thumbs__img--active")});
...
_elements.scrollLinks.forEach(link => {
	link.addEventListener("click", e => {
		...
		const id = link.getAttribute("href"); const element = document.querySelector(id);
		const position = element.offsetTop - 40;
		window.scrollTo({top: position, behavior: "smooth"}); e.preventDefault();
	})
});
